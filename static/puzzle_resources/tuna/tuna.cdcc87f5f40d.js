...
function selectSong(i) {
  ...
  if (currentSong === i && isPlaying) {
    ...
    audio.addEventListener('canplaythrough', () => { 
      endTimestamp = Date.now() + (MUSIC[i][2] - numSecsProgress)*SEC_TO_MILLIS;
...
document.body.addEventListener('mousemove', (e) => {
  if (isDraggingProgressHandle) {
    ...
    const numSecsProgress = Math.floor(fractionProgress * MUSIC[currentSong][2]);
...
document.body.addEventListener('mouseup', (e) => {
  if (isDraggingProgressHandle) {
    ...
    const deltaX = e.clientX - rect.left;
    const fractionProgress = deltaX / rect.width;
    const numSecsProgress = fractionProgress * MUSIC[currentSong][2];
    endTimestamp = Date.now() + (MUSIC[currentSong][2] - numSecsProgress)*SEC_TO_MILLIS;
    audio.currentTime = numSecsProgress;
    selectSong(currentSong);
...
function updateState() {
  if (isPlaying) {
    ...
    const numSecsProgress = MUSIC[currentSong][2] - numSecUntilEnd;
    const percentageProgress = TO_PERCENT * numSecsProgress / MUSIC[currentSong][2];
    playbackPosition.innerText = formatTime(Math.floor(numSecsProgress));
    progressBarStyleHolder.style = `--progress-bar-transform:${percentageProgress}%;`;
  }
}
